        ...
        <div className="flex justify-between items-center px-4 md:px-6 py-4 bg-[var(--color-surface-glass)] backdrop-blur-md border-b border-[var(--color-border-glass)] shadow-sm shrink-0 z-20">
          <div className="flex items-center gap-4">
             ...
             <div className="flex items-center gap-3">
                <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-[var(--color-secondary)] to-purple-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-purple-500/20">
                   <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:h-5 md:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                   </svg>
                </div>
                <div>
                    <h2 className="text-lg md:text-xl font-extrabold text-[var(--color-text-main)] tracking-tight">דו"ח אנליזה חכם</h2>
                    <p className="text-[10px] md:text-xs text-[var(--color-text-muted)] font-medium hidden sm:block">
          ...
          <div className="flex items-center gap-2">
            {analysisResult && onSave && (
              <button
                ...
                disabled={isSaved}
                className={`flex items-center gap-2 px-3 py-2 md:px-5 md:py-2.5 rounded-xl text-xs md:text-sm font-bold transition-all shadow-sm
                   ${isSaved 
              ...
              >
                 {isSaved ? (
                   <>
                     ...
                     <span className="hidden md:inline">נשמר במועדפים</span>
                   </>
                 ) : (
                   <>
                     <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                     <span className="hidden md:inline">שמור דוח</span>
            ...
            {loading ? (
               <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center">
                  <div className="relative">
                    <div className="w-16 h-16 md:w-24 md:h-24 border-4 border-[var(--color-border-glass)] border-t-[var(--color-secondary)] rounded-full animate-spin"></div>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-xl md:text-3xl animate-pulse">✨</span>
                    </div>
                  </div>
                  <div className="animate-pulse px-4">
                     <h3 className="text-xl md:text-2xl font-bold text-[var(--color-text-main)] mb-2">מבצע ניתוח מעמיק...</h3>
                     <p className="text-sm md:text-base text-[var(--color-text-muted)] max-w-md mx-auto">ה-AI סורק את הנתונים שלך, מחשב מדדים ומחפש תובנות עסקיות משמעותיות.</p>
                  </div>
               </div>
            ) : analysisResult ? (
              <div className="space-y-6 md:space-y-8 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                 
                 {/* 1. Executive Summary */}
                 <div className="bg-[var(--color-surface-glass)] backdrop-blur-xl p-6 md:p-8 rounded-3xl shadow-lg border border-[var(--color-border-glass)] relative overflow-hidden group">
                    ...
                    <h3 className="text-lg md:text-xl font-bold text-[var(--color-text-main)] mb-4 flex items-center gap-3 relative z-10">
                    ...
                    <p className="text-[var(--color-text-main)]/80 leading-relaxed text-base md:text-lg relative z-10 max-w-4xl">{analysisResult.summary}</p>
                 </div>
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                   {analysisResult.charts.map((chart, idx) => (
                     <div 
                        key={idx} 
                        className="bg-[var(--color-surface-glass)] backdrop-blur-md p-6 md:p-8 rounded-3xl shadow-sm border border-[var(--color-border-glass)] relative group transition-all hover:shadow-xl hover:border-[var(--color-primary)]/20"
                     >
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                <h4 className="font-bold text-[var(--color-text-main)] text-lg md:text-xl">{chart.title}</h4>
                                <p className="text-sm text-[var(--color-text-muted)] mt-1">{chart.description}</p>
                            </div>
                            {onDrillDown && (
                                <button 
                                    onClick={() => onDrillDown(analysisResult.sourceEntity)}
                                    className="opacity-100 md:opacity-0 group-hover:opacity-100 bg-[var(--color-surface-200)] text-[var(--color-primary)] p-2 rounded-lg text-xs font-bold transition-all hover:bg-[var(--color-primary)] hover:text-white flex items-center gap-1"
                                    title="Drill Down"
                                >
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                                    <span className="hidden md:inline">חקור נתונים</span>
                                </button>
                            )}
                        </div>
